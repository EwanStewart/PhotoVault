...
# Driven by the deploy workflow; safe to call manually.
...
# The deploy runs inside a function so bash parses the whole file before
# executing any of it. git reset replaces this script on disk mid-run;
# without the wrapper bash keeps reading stale byte offsets from the new file.
main() {
    cd "$REPO"
