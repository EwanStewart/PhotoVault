...
def organise(remote, photos):
    ...
    A location whose name changes, as happens when a place name is
    refetched in English, empties its old folder over several passes.
    One sweep after the moves clears whatever is left behind.
    """
    moved = 0
    ...
    if moved:
        drive.remove_empty_dirs(remote)
    return moved
